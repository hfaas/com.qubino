...
class QubinoDevice extends ZwaveDevice {
	async onMeshInit() {
		...
		this._inputs = {
			2: {
				state: null,
				flowTriggers: {
					on: constants.flows.inputTwoTurnedOn,
					off: constants.flows.inputTwoTurnedOff,
					toggle: constants.flows.inputTwoToggled,
				},
			},
			3: {
				state: null,
				flowTriggers: {
					on: constants.flows.inputThreeTurnedOn,
					off: constants.flows.inputThreeTurnedOff,
					toggle: constants.flows.inputThreeToggled,
				},
			},
		};
		...
		if (Array.isArray(inputSensorEndpoints) && inputSensorEndpoints.length > 0) {
			if (inputSensorEndpoints.length === 1) {
				if (this.inputTwoEnabled === true) {
					this._registerInputEndpointListener(inputSensorEndpoints[0], 2);
				} else if (this.inputTwoEnabled === false) {
					this._registerInputEndpointListener(inputSensorEndpoints[0], 3);
				}
			} else {
				inputSensorEndpoints.forEach(inputSensorEndpoint => {
					this._registerInputEndpointListener(inputSensorEndpoint, inputSensorEndpoint);
				});
			}
		}
	}

	_registerInputEndpointListener(inputSensorEndpoint, inputId) {
		...
		this.registerMultiChannelReportListener(
			...
			(...args) => this.processInputEvent(inputId, ...args));
	 ...
	 * @private
	 */
	processInputEvent(inputId, report) {
		if (!inputId) throw new Error('missing_endpoint_id');
		...
		// Get input object
		const inputObj = this._inputs[inputId];
		if (!inputObj) throw new Error(`unknown_input_id_${inputId}`);
		if (inputObj.state === newState) return; // Do nothing when state did not change
		inputObj.state = newState;

		this.log(`Received notification from input ${inputId}: ${newState}`);
	...
	async onSettings(oldSettings, newSettings, changedKeysArr) {
		...
		if (changedKeysArr.includes(constants.settings.allOn) || changedKeysArr.includes(constants.settings.allOff)) {
			...
			await this.configurationSet({
				...
				size: constants.settings.size.allOnAllOff,
			}, allOnAllOf);
