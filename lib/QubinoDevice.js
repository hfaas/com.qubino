...
class QubinoDevice extends ZwaveDevice {
	async onMeshInit() {
	  this.enableDebug(); // TODO: remove this when this app version becomes stable
		...
		this.log('Is migrated:', !!isMigrated);
	...
	registerTemperatureSensorEndpoint() {
      ...
      this.registerCapability(constants.capabilities.measureTemperature, constants.commandClasses.sensorMultilevel, {
		...
		} else if (this.hasCommandClass(constants.commandClasses.sensorMultilevel) && this.hasCapability(constants.capabilities.measureTemperature)) { // command class on root node
      this.log('Configured temperature sensor on root node', temperatureSensorEndpoint);
      this.registerCapability(constants.capabilities.measureTemperature, constants.commandClasses.sensorMultilevel);
    } else if (this.hasCapability(constants.capabilities.measureTemperature)) {
