...
const { CAPABILITIES, SETTINGS, COMMAND_CLASSES, DEVICE_CLASS_GENERIC, INPUT_MAP } = require('./constants');
   ...
   */
	...
	_migrateSettings() {
		...
		currentSettingsKeys.forEach(settingKey => {
			...
			let manifestSetting = this.getManifestSetting(settingKey);
	...
	findInputSensorEndpoints() {
		...
		for (const i in this.node.MultiChannelNodes) {
			if (this.node.MultiChannelNodes[i].deviceClassGeneric === DEVICE_CLASS_GENERIC.SENSOR_BINARY ||
				this.node.MultiChannelNodes[i].deviceClassGeneric === DEVICE_CLASS_GENERIC.SENSOR_NOTIFICATION) {
	...
	async registerInputs() {
		...
		for (const inputConfig of inputConfigs) {
			if (!inputConfig.hasOwnProperty('DEFAULT_ENABLED') || inputConfig.DEFAULT_ENABLED === false) {
