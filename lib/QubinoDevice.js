...
class QubinoDevice extends ZwaveDevice {
	...
	/**
	 * This is a map off often used settings and migrations.
	...
	_genericMigrationMap() {
		return {
			switchTypeInput1: this.getSetting('input_1_type'),
			switchTypeInput2: this.getSetting('input_2_type'),
			contactTypeInput2: this.getSetting('input_2_contact_type'),
			contactTypeInput3: this.getSetting('input_3_contact_type'),
			autoOn: Math.min(this.getSetting('automatic_turning_on_output_after_set_time'), 32535),
			autoOff: Math.min(this.getSetting('automatic_turning_off_output_after_set_time'), 32535),
			restoreStatus: !this.getSetting('state_of_device_after_power_failure'),
			powerReportingThreshold: this.getSetting('power_report_on_power_change'),
			powerReportingInterval: Math.min(this.getSetting('power_report_by_time_interval'), 32535),
			maximumDimValue: this.getSetting('maximum_dimming_value'),
			minimumDimValue: this.getSetting('minimum_dimming_value'),
			dimDuration: Math.max(Math.min(this.getSetting('dimming_time_soft_on_off') / 100, 2.5), 0.5),
			dimDurationKeyPressed: this.getSetting('dimming_time_when_key_pressed'),
		}
	}

	_migrateSettings() {
		let settingsMigrationMap = {...this._genericMigrationMap(), ...this._settingsMigrationmap()};
		this.setSettings(settingsMigrationMap);
