...
class QubinoDriver extends Homey.Driver {
	...
	onInit() {
		[
			...
			{
				key: constants.flows.setOffAutoThermostatMode,
	...
	_registerFlowCardAction(key, fn) {
		...
		try {
			...
			this.log('registerFlowCardAction() -> registered', `${key}_${this.id}`);
		} catch (err) {
	...
	_registerThermostatModeChangedFlowCardTriggerDevice() {
		if (!this.flowCardTriggerDevice) this.flowCardTriggerDevice = {};
		try {
			this.flowCardTriggerDevice[constants.flows.offAutoThermostatModeChanged] = new Homey.FlowCardTriggerDevice(`${constants.flows.offAutoThermostatModeChanged}_${this.id}`)
				.registerRunListener((args, state) => args.mode && state.mode && args.mode.toLowerCase() === state.mode.toLowerCase())
				.register();
			this.log('_registerThermostatModeChangedFlowCardTriggerDevice() -> registered', `${constants.flows.offAutoThermostatModeChanged}_${this.id}`);
		} catch (err) {
			// this.error(`failed to register flow card trigger device ${constants.flows.offAutoThermostatModeChanged} for ${this.id}`, err.message);
		}
	}
