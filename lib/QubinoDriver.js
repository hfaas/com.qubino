...
class QubinoDriver extends Homey.Driver {
	...
	_registerFlowCardAction(key, fn) {
		...
		try {
			const card = Homey.ManagerFlow.getCard('action', `${key}_${this.id}`); // TODO: check if this works
			if (card instanceof Error) return;
	...
	_registerFlowCardTriggerDevice(key) {
		...
		try {
			const card = Homey.ManagerFlow.getCard('trigger', `${key}_${this.id}`); // TODO: check if this works
			if (card instanceof Error) return;
